...
DEPENDENCIES="c_greatest_dep c_vector_dep c_fsio_dep c_stringfn_dep debug_memory_dep"
...
MESON_CMDS=" && meson setup --reconfigure build && meson compile -C build $TEST_CASE_NAME && meson test --print-errorlogs -v -C build $TEST_CASE_NAME"
echo -n "" >$JO_ARGS_FILE
...
J2_POST_CMDS="$UNCRUSTIFY_BIN -l c -q -c $UNCRUSTIFY_CFG $TEST_CASE_RENDERED_C_FILE $TEST_CASE_RENDERED_H_FILE && muon check $TEST_CASE_RENDERED_MESON_BUILD_FILE && { [[ -d "$TEST_CASE_DIR" ]] || mkdir $TEST_CASE_DIR; } && cp $TEST_CASE_RENDERED_C_FILE $TEST_CASE_RENDERED_H_FILE $TEST_CASE_RENDERED_MESON_BUILD_FILE $TEST_CASE_DIR $MESON_CMDS"
