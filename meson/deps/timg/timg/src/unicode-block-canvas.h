...
public:
  ...
  UnicodeBlockCanvas(BufferedWriteSequencer *ws, bool use_quarter, bool use_upper_half_block, bool use_256_color);
  ~UnicodeBlockCanvas() override;

  void Send(int x, int dy, const Framebuffer &framebuffer, SeqType seq_type, Duration end_of_frame) override;
...
private:
  ...
  char *AppendDoubleRow(char *pos, int indent, int width, const rgba_t *top_line, const rgba_t *bottom_line, bool emit_difference, int *y_skip);
