...
public:
  ...
  bool LoadAndScale(const DisplayOptions &options, int frame_offset, int frame_count) final;
  ...
  void SendFrames(const Duration &duration, int loops, const volatile sig_atomic_t &interrupt_received, const Renderer::WriteFramebufferFun &sink) final;
...
private:
  ...
  void Scroll(const Duration &duration, int loops, const volatile sig_atomic_t &interrupt_received, int dx, int dy, const Duration &scroll_delay, const Renderer::WriteFramebufferFun &write_fb);
