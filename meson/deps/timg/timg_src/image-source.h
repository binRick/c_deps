...
public:
  ...
  static ImageSource *Create(const std::string &filename, const DisplayOptions &options, int frame_offset, int frames_count, bool attempt_image_loading, bool attempt_video_loading, bool print_errors);
  ...
  virtual void SendFrames(const Duration &duration, int loops, const volatile sig_atomic_t &interrupt_received, const Renderer::WriteFramebufferFun &sink) = 0;
...
protected:
  ...
  // factory.
  virtual bool LoadAndScale(const DisplayOptions &options, int frame_offset, int frame_count) = 0;
  ...
  static bool CalcScaleToFitDisplay(int img_width, int img_height, const DisplayOptions &display_options, bool fit_in_rotated_frame, int *target_width, int *target_height);

  // Utility function to format
  static std::string FormatFromParameters(const std::string &fmt_string, const std::string &filename, int orig_width, int orig_height, const char *decoder);
