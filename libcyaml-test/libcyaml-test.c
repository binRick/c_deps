...
#include "libcyaml-test/libcyaml-test.h"
...
int libcyaml_main(char *INPUT_FILE, char *OUTPUT_FILE){
  cyaml_err_t err;
  struct plan *plan;
  ...
  err = cyaml_load_file(INPUT_FILE, &config,
  ...
  err = cyaml_save_file(OUTPUT_FILE, &config,
                        &plan_schema, plan, 0);
  if (err != CYAML_OK) {
    fprintf(stderr, "ERROR: %s\n", cyaml_strerror(err));
...
TEST t_libcyaml_test2(){
  PASS();
}

TEST t_libcyaml_test1(){
  char *INPUT_FILE;

  asprintf(&INPUT_FILE, "%s/../../libcyaml-test/project.yaml", EXECUTABLE_PATH_DIRNAME);
  char *OUTPUT_FILE = "/tmp/project-out.yaml";

  log_info("%s", INPUT_FILE);
  int rc = libcyaml_main(INPUT_FILE, OUTPUT_FILE);
